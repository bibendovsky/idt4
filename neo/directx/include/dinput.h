...
typedef BOOL(FAR PASCAL* LPDIENUMDEVICESBYSEMANTICSCBA)(
	LPCDIDEVICEINSTANCEA, LPDIRECTINPUTDEVICE8A, DWORD, DWORD, LPVOID);
typedef BOOL(FAR PASCAL* LPDIENUMDEVICESBYSEMANTICSCBW)(
	LPCDIDEVICEINSTANCEW, LPDIRECTINPUTDEVICE8W, DWORD, DWORD, LPVOID);
#ifdef UNICODE
...
DECLARE_INTERFACE_(IDirectInput8W,IUnknown)
{
	...
	STDMETHOD(EnumDevicesBySemantics)(
		THIS_ LPCWSTR, LPDIACTIONFORMATW, LPDIENUMDEVICESBYSEMANTICSCBW, LPVOID, DWORD) PURE;
	STDMETHOD(ConfigureDevices)(
		THIS_ LPDICONFIGUREDEVICESCALLBACK, LPDICONFIGUREDEVICESPARAMSW, DWORD, LPVOID) PURE;
};
...
DECLARE_INTERFACE_(IDirectInput8A,IUnknown)
{
	...
	STDMETHOD(FindDevice)(THIS_ REFGUID, LPCSTR, LPGUID) PURE;
	STDMETHOD(EnumDevicesBySemantics)(
		THIS_ LPCSTR, LPDIACTIONFORMATA, LPDIENUMDEVICESBYSEMANTICSCBA, LPVOID, DWORD) PURE;
...
/*
 * The device is a polled device.  As a result, device buffering will not collect any data and
 * event notifications will not be signalled until GetDeviceState is called.
 */
#define DI_POLLEDDEVICE                 ((HRESULT)0x00000002L)

/*
 * The parameters of the effect were successfully updated by IDirectInputEffect::SetParameters,
 * but the effect was not downloaded because the device is not exclusively acquired or because
 * the DIEP_NODOWNLOAD flag was passed.
 */
#define DI_DOWNLOADSKIPPED              ((HRESULT)0x00000003L)

/*
 * The parameters of the effect were successfully updated by IDirectInputEffect::SetParameters,
 * but in order to change the parameters, the effect needed to be restarted.
 */
#define DI_EFFECTRESTARTED              ((HRESULT)0x00000004L)

/*
 * The parameters of the effect were successfully updated by IDirectInputEffect::SetParameters,
 * but some of them were beyond the capabilities of the device and were truncated.
 */
 ...
 * or the object was not in a state that admitted the function to be called.
 */
...
/*
 * The effect could not be downloaded because essential information is missing.  For example,
 * no axes have been associated with the effect, or no type-specific information has been created.
 */
...
/*
 * An attempt was made to modify parameters of an effect while it is playing.  Not all
 * hardware devices support altering the parameters of an effect while it is playing.
 */
 ...
 * Some devices have restrictions on how much data can be sent to them. (For example, there might be a
 * limit on the number of buttons that can be pressed at once.)
 */
...
/*
 * "ANY" semantics can be used as a last resort to get mappings for actions that match nothing in the chosen
 * virtual genre. These semantics will be mapped at a lower priority that virtual genre semantics. Also, hardware
 * vendors will not be able to provide sensible mappings for these unless they provide application specific mappings.
 */
...
/*
 * Flag to indicate that the dwReserved2 field of the JOYINFOEX structure contains mini-driver specific data
 * to be passed by VJoyD to the mini-driver instead of doing a poll.
 */
#define JOY_PASSDRIVERDATA          0x10000000L

/*
 * Informs the joystick driver that the configuration has been changed and should be reloaded from the registry.
...
#ifndef DIJ_RINGZERO
/*
 * Invoke the joystick control panel directly, using the passed window handle as the parent of the dialog.
 * This API is only supported for compatibility purposes; new applications should use the RunControlPanel method of a
 * device interface for a game controller. The API is called by using the function pointer returned by
 * GetProcAddress(hCPL, TEXT("ShowJoyCPL")) where hCPL is a HMODULE returned by LoadLibrary(TEXT("joy.cpl")).
 * The typedef is provided to allow declaration and casting of an appropriately typed variable.
 */
...
/* Hardware Setting indicating that the VxD is used to replace the standard analog polling */
#define JOY_HWS_ISGAMEPORTDRIVER    0x04000000L

/* Hardware Setting indicating that the driver needs a standard gameport in order to communicate with the device. */
#define JOY_HWS_ISANALOGPORTDRIVER  0x08000000L

/*
 * Hardware Setting indicating that VJoyD should not load this driver, it will be loaded externally and
 * will register with VJoyD of it's own accord.
 */
...
/* Usage Setting indicating that the settings are volatile and should be removed if still present on a reboot. */
